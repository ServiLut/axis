...
// Based on what we saw in the page and mysql.ts
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Detalles completos del servicio prestado
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
                ...
                <div className="bg-slate-50 p-4 rounded-lg space-y-3">
                    ...
                    <div>
                        ...
                        <div className="flex flex-col mt-1 text-sm text-slate-500 gap-1">
                            ...
                            {(servicio.numero_piso || servicio.bloque_o_torre || servicio.unidad_residencial) && (
                                <div className="pl-5 text-xs">
                                    {servicio.unidad_residencial && <span className="block">Unidad: {servicio.unidad_residencial}</span>}
                                    {servicio.bloque_o_torre && <span className="block">Torre/Bloque: {servicio.bloque_o_torre}</span>}
                                    {servicio.numero_piso && <span className="block">Piso/Apto: {servicio.numero_piso}</span>}
                                </div>
                            )}
                ...
                <div className="bg-slate-50 p-4 rounded-lg space-y-3">
                    ...
                    <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium">{servicio.numero_orden || "N/A"}</span>
                        </div>
                         <div className="flex justify-between">
                            <span className="text-slate-500">Zona Locativa:</span>
                            <span className="font-medium">{servicio.zonas_locativas?.zona || "N/A"}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div className="space-y-2 text-sm border-l-2 border-indigo-100 pl-3">
                        ...
                        <div className="grid grid-cols-3 gap-2 mt-2">
                             <div>
                                ...
                                <span className="font-medium text-slate-700">{formatTime(servicio.hora_entrada)}</span>
                            </div>
                            <div>
                                <span className="block text-xs text-slate-500 uppercase tracking-wide flex items-center">
                                    <Clock className="h-3 w-3 mr-1" /> Salida
                                </span>
                                <span className="font-medium text-slate-700">{formatTime(servicio.hora_salida)}</span>
                            </div>
                            <div>
                                <span className="block text-xs text-slate-500 uppercase tracking-wide">Duración</span>
                                <span className="font-medium text-slate-700">{servicio.duracion_servicio || "-"}</span>
                            </div>
                        </div>
                    </div>
                 </div>
            ...
            <Separator />
             ...
             <div className="space-y-4">
                  ...
                  servicio.observacion_final) && (
                    <div>
                        ...
                        <div className="space-y-2">
                             {servicio.observaciones && (
                                <div className="bg-amber-50 text-amber-900 p-3 rounded-md text-sm border border-amber-100">
                             ...
                             {servicio.observaciones_coti && (
                                <div className="bg-purple-50 text-purple-900 p-3 rounded-md text-sm border border-purple-100">
                                    <span className="font-semibold block mb-1 text-xs uppercase opacity-70">Técnico Cotización:</span>
                                    {servicio.observaciones_coti}
                                </div>
                             )}
                             {servicio.observacion_cotizacion && (
                                <div className="bg-indigo-50 text-indigo-900 p-3 rounded-md text-sm border border-indigo-100">
                                    <span className="font-semibold block mb-1 text-xs uppercase opacity-70">Cotización:</span>
                                    {servicio.observacion_cotizacion}
                                </div>
                             )}
                             {servicio.observacion_final && (
                                <div className="bg-slate-50 text-slate-700 p-3 rounded-md text-sm border border-slate-200">
                                     <span className="font-semibold block mb-1 text-xs uppercase opacity-70">Final:</span>
                                    {servicio.observacion_final}
                                </div>
                             )}
                        </div>
                ...
                {servicio.que_se_hizo && (
                    <div>
                        <h3 className="text-sm font-semibold text-slate-900 mb-2">¿Qué se hizo?</h3>
                 ...
                 {servicio.implementos && (
                    <div>
                        <h3 className="text-sm font-semibold text-slate-900 mb-2">Implementos Utilizados</h3>
                 ...
                 <>
                    ...
                    <div>
                         ...
                         <div className="border border-slate-200 rounded-lg p-2 bg-white flex justify-center">
                             {/* Assuming firma_cliente is a base64 string or URL. If it's base64, it usually needs the prefix data:image/... */}
                             <Image 
