...
export default async function ServTecnicoClientesPage({
  searchParams,
}: {
  ...
  // Fetch data
  const { data: clientes, total } = await tecnicos.clientes.findMany({
    orderBy: {
      Id_cliente: 'desc'
    },
    take: itemsPerPage,
  ...
  const { municipios } = await tecnicos.clientes.getFilterOptions();
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-2xl font-bold text-slate-900">
                Clientes Serv. Técnico
              </h1>
            </div>
            <p className="text-sm text-slate-600 mt-1">
              Base de datos histórica de clientes y su información de contacto (Técnicos)
            </p>
          </div>
        </div>
      </div>
