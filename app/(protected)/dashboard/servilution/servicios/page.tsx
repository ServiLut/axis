...
import { ServiceActions, type ServiceData } from "@/components/dashboard/servilution/service-actions";

export default async function ServilutionServiciosPage({
  searchParams,
}: {
  ...
  // Fetch data
  const { data: servicios, total } = await mysql.servicios_prestados.findMany({
    orderBy: { id: 'desc' },
    take: itemsPerPage,
    skip: skip,
    where: where
  ...
  const { empresas, municipios, tecnicos } = await mysql.servicios_prestados.getFilterOptions();
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 max-w-7xl mx-auto">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">
              Servicios Servilution
            </h1>
            <p className="text-sm text-slate-600 mt-1">
              Historial de servicios prestados y gestión
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:flex-nowrap gap-3 items-center">
          ...
          <div className="flex flex-wrap md:flex-nowrap gap-2 w-full md:w-auto items-center justify-end">
            ...
            <FilterSelect 
                ...
                options={(tecnicos || []).map((t) => ({ value: t.id.toString(), label: `${t.nombre} ${t.apellido}` }))}
                className="w-full md:w-[150px] bg-white"
            />
