...
import type { clientes as Cliente } from "../../../../../prisma/generated/prisma-mysql/client";
...
export async function getClienteDetails(id: number) {
  try {
    ...
    const [cliente] = await mysql.query<Cliente & { nombre_municipio: string, nombre_departamento: string, nombre_barrio: string }>(`
    ...
    const { data: servicios } = await mysql.servicios_prestados.findMany({
      where: {
        id_cliente: id
      },
